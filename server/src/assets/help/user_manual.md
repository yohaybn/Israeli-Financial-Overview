...
Insight rules are **deterministic**: the server evaluates conditions against your stored transactions (no LLM call when a rule fires). Each rule has **two message strings**—English and Hebrew—so the dashboard and APIs can show the right language. **Placeholders** in the text are replaced when the rule matches, for example: `{{sum}}`, `{{count}}`, `{{category}}`, `{{period_label}}`, `{{sum_raw}}`, `{{threshold}}`, `{{avg_txn}}`, `{{pct_of_total}}`, `{{top_memo}}`, `{{largest_txn_amount}}`, `{{largest_txn_date}}`, `{{largest_txn_memo}}`, `{{dominant_account}}`, `{{delta_prior}}`, `{{currency}}`, `{{preview_3}}`. The **Configuration → Insight rules** screen includes an **Engine reference** panel with short descriptions and a sample JSON condition.
...
*   **What a rule contains:** A JSON **definition** (version 1) with:
    ...
    *   **condition:** A tree of logical operators (`and`, `or`, `not`), plus building blocks such as:
        *   Expense totals: `sumExpensesGte`, `sumExpensesLte`, `sumExpensesBetween` (min/max, optional category).
        *   Expense counts: `txnCountGte`, `txnCountBetween`.
        *   Income totals: `sumIncomeGte`, `sumIncomeLte` (positive amounts, optional category).
        *   Outliers / structure: `maxSingleExpenseGte` (largest single expense in scope), `shareOfCategoryGte` (category’s share of **all** expenses, `share` is 0–1), `netSavingsLte` (income sum − expense sum ≤ threshold).
        *   “At least one transaction matches”: `existsTxn` with **TxnCondition** checks: category equals / in list, memo or description contains, account, ignored flag, `amountAbsGte` / `amountAbsLte` / `amountAbsBetween`, **day of week** (`dayOfWeekIn` with 0=Sunday … 6=Saturday, local calendar day), `isExpense`, `isIncome`, and nested `and` / `or` / `not`.
    *   **output:** `kind` (`insight` or `alert`), **score** (1–100 for ranking next to AI insights), and **message** `{ "en": "...", "he": "..." }`.
*   **Creating and editing:** Expand **Add rule** under the list to open the same **If / Then** form in place. Optionally use **get a draft from AI** at the top of that form (requires a configured Gemini API key): describe what you want in plain language, then refine the result before saving. AI-suggested drafts are labeled **AI** in the list and can start disabled until you enable them.
...
*   **Export / import:** **Export JSON** downloads a file with `format` `financial-overview-insight-rules` and `version` 1. **Import from pasted export JSON** lets you paste that document, then use **Parse & continue** to open a **review** step: adjust built-in fields such as category names, amounts, or percentages so the imported rule matches *your* categories and scale before anything is saved. Confirm when ready. Choose **merge** (upsert by rule id) or clear-and-replace by turning merge off for the final import.
*   **Share to community catalog:** On each saved rule, the **share** action opens a short form: how you want to be credited, and an optional note for people reviewing submissions. **Submit** sends your rule for inclusion in the shared catalog when that option is enabled for your installation; if it is not, the panel explains that you cannot send from here. After a successful submission, you see a confirmation.
*   **Community catalog (browse & add):** In the **Community catalog** section under the same tab, refresh the list to see published rules (including **Featured** highlights when present). **Import** on a row downloads that rule and opens it in a **full-screen style editor**—the same fields as **Add rule** (name, enabled, priority, If/Then, advanced JSON)—so you can tweak everything before **Save** adds a **copy** to your own rules (new entries start **enabled**). **Advanced settings** there are only needed if you use an alternate catalog source provided by your organization; otherwise the built-in defaults apply.
