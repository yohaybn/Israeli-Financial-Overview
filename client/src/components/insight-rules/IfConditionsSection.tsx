...
function TxnMatchFields({
    ...
    return (
        <div className="flex flex-col gap-2 ms-2 border-s-2 border-indigo-100 ps-3">
            ...
            {(op === 'categoryEquals' || op === 'memoOrDescriptionContains' || op === 'accountEquals') && (
                <input
                    type="text"
                    disabled={disabled}
                    ...
                    onChange={(e) => {
                        ...
                        if (op === 'categoryEquals') onChange({ op: 'categoryEquals', value: v });
                        else if (op === 'memoOrDescriptionContains') onChange({ op: 'memoOrDescriptionContains', value: v });
    ...
    return (
        <div className="rounded-lg border border-gray-200 bg-white p-3 space-y-2">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                    ...
                    <div>
                        ...
                        <input
                            ...
                            className="mt-1 w-full rounded-lg border border-gray-200 p-2 text-sm"
                            placeholder={t('insight_rules.all_categories')}
                        />
                    </div>
                </div>
            )}
            ...
            {row.rowType === 'txn_exists' && (
                <TxnMatchFields match={row.match} disabled={disabled} onChange={(m) => onChange({ ...row, match: m })} />
            )}
        </div>
    );
}
        ...
        <section className="space-y-3" aria-labelledby="if-heading">
            ...
            <div>
                ...
                <div className="flex flex-wrap gap-2">
                    {(['sum_expenses', 'txn_count', 'txn_exists'] as const).map((kind) => (
                        <button
                            key={kind}
                            type="button"
                            disabled={disabled}
                            onClick={() => setRows([...state.rows, newRow(kind)])}
                            className="rounded-lg border border-dashed border-indigo-200 bg-white px-3 py-1.5 text-xs font-medium text-indigo-700 hover:bg-indigo-50"
                        >
                            + {t(`insight_rules.param_${kind}` as const)}
                        </button>
                    ))}
                </div>
            </div>
